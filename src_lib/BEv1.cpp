...
plist BXXSMSM[2][2];
plist AntiBXXSMSM[2][2];

plist BXSMXSM[2][2];
plist AntiBXSMXSM[2][2];


plist XSMSMSM[2];
plist BXSMSMSM[2];
plist AntiBXSMSMSM[2];
...
void facB(int &pid,pair <double,double> &fac,double const &T)
{
    ...
    {
        pp.push_back(make_pair(found,0.5*sp*mu_u));
    ...
    while (found!=std::string::npos)
    {
        pp.push_back(make_pair(found,0.5*sp*mu_d));
        ...
        for(size_t jj = 0;jj!=pXSM.size();++jj){
            ...
            for(size_t kk = 0;kk!=pthSM.size();++kk){
                for(size_t ll = 0;ll!=pthSM.size();++ll){
                    ...
                    pX.push_back(pXSM[ii]);
                    pX.push_back(pXSM[jj]);
                    pX.push_back(pthSM[kk]);
                    pX.push_back(pthSM[ll]);
...
void functh(param_t &pp,double hh,double zl,Eigen::VectorXcd &yn)
{
    ...
    for(size_t pi = 0;pi!=pXSM.size();++pi){
        double gg = 1.;
        for(auto &gi : gptl){
            gg *= (pXSM[pi].find(gi.first)!=std::string::npos) ? gi.second : 1.;
        }
        yeq(pi) = Yeq(gg,prtlm(pXSM[pi],pp),T);
        ...
        for(size_t ii= 0;ii!=pXSM.size();++ii){
            for(size_t jj= 0;jj!=pXSM.size();++jj){
                //if(ii<=jj)
                {
                    ...
                    GCPOdd2(ii,jj) += abs(XsecB-XsecAB);
...
void diff(std::ofstream &file,double zmax,double zin,param_t &pp)
{
    ...
    double T = m_XX(pp).real()/pow(10., z);
    
    size_t tot = pXSM.size() + 1;
    
    cout << "Tot =" << tot << endl;
    Eigen::VectorXcd yeq(tot),yn(tot);
    ...
    for(size_t pi = 0;pi!=pXSM.size();++pi){
        ...
        yn(pi) = Yeq(gg,prtlm(pXSM[pi],pp),T);
        yeq(pi) = Yeq(gg,prtlm(pXSM[pi],pp),T);
    ...
    */
    for(i=0;i<iter;i++)
    {
        T = m_XX(pp).real()/pow(10., z);
        
        for(size_t pi = 0;pi!=pXSM.size();++pi){
            double gg = 1.;
            for(auto &gi : gptl){
                gg *= (gi.first==pXSM[pi]) ? gi.second : 1.;
            }
            yeq(pi) = Yeq(gg,prtlm(pXSM[pi],pp),T);
        }
        yeq(tot-1) = Yeq(2.,0.,T);
        
        functh(pp,h,z,yn);  //printf("\n");
        z = z + h;
