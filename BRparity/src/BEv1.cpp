...
void facB(int& pid, pair<double, double>& fac, double const& T)
{
    ...
    mu_u += (T > 100.) ? -10. / 28.
                       : 6. * (1. / 3. + 0.5 / Nd + 0.5 / Ne) /
                           (1. + 3. * Nu / Ne + Nu / Nd + 2. * Nu);

    mu_d +=
      (T > 100.) ? 38. / 28. : 6. * (1. - 1. / 3. * Nu * mu_u) / (2. * Nd);
...
std::string Prname(vector<std::string> const& ptln,
                   int const&                 inp,
                   int const&                 outp)
{
    std::string res = "CP";
...
double geffT(double x)
{
    ...
    double xL = temp[i], yL = geff[i], xR = geff[i + 1],
           yR = geff[i + 1]; // points on either side (unless beyond ends)
    if (x < xL)
        yR = yL;
    if (x > xR)
        yL = yR;
...
double Yeq(double gi, double mi, double T)
{
    double yy = 0.;
    if (mi > 0.) {
        yy = 45. * gi / geffT(T) * pow(mi / (2. * M_PI * T), 2.) *
             boost::math::cyl_bessel_k(2, mi / T);
    }
    if (mi == 0.) {
        yy =
          45. / 2. * gi / geffT(T) * pow(M_PI, -4.); // cout << "h1!!" << endl;
    }
    return yy;
}
...
Eigen::MatrixXcd Mass(Eigen::VectorXcd& th, Eigen::VectorXcd& mm)
{
    ...
    O12 << cos(th(0)), -sin(th(0)), 0., sin(th(0)), cos(th(0)), 0., 0., 0., 1.;
    O13 << cos(th(1)), 0., -sin(th(1)), 0., 1., 0., sin(th(1)), 0., cos(th(1));
    O23 << 1., 0., 0., 0., cos(th(2)), -sin(th(2)), 0., sin(th(2)), cos(th(2));
...
{
    return xx * xx + yy * yy + zz * zz - 2. * xx * yy - 2. * xx * zz -
           2. * yy * zz;
}

double jac(complex_t             sqss,
           Eigen::VectorXcd&     mm,
           ...
           int const&            outp)
{
    ...
    if ((intp == 1) && (outp == 3)) {
        ...
        res = 0.5 / mD * 1. / (4. * M_PI) *
              sqrt(L(mD * mD, m1 * m1, s23).real()) / (8. * M_PI * mD * mD) *
              sqrt(L(s23, m2 * m2, m3 * m3)).real() / (8. * M_PI * s23);
    }
    if ((intp == 1) && (outp == 2)) {
        double mD = mm(0).real();
        double m1 = mm(1).real();
        ...
        res = 1. / (16. * M_PI * pow(mD, 3.)) *
              sqrt(L(mD * mD, m1 * m1, m2 * m2)).real();
    }
    if ((intp == 2) && (outp == 2)) {
        ...
        double    m2 = mm(2).real();
        double    m3 = mm(3).real();
        res          = 1. / (64. * M_PI * M_PI * ss.real()) *
              sqrt(L(ss, m2 * m2, m3 * m3)).real() /
              sqrt(L(ss, ma * ma, mb * mb)).real();
    }
    return res;
}

Eigen::MatrixXcd pmom(complex_t             sqss,
                      Eigen::VectorXcd&     mm,
                      ...
                      int const&            outp)
{
    ...
    for (int ii = 0; ii != 4; ++ii) {
        pmom[ii] = Eigen::VectorXcd::Zero(3);
    }
    ...
    if ((inp == 1) && (outp == 3)) {
        ...
        pmom[0] << mD, 0., 0.;
        pmom[1] << (sqrt(pow(mD, 2)) *
                    (1 + pow(m1, 2) / pow(mD, 2) - s23 / pow(mD, 2))) /
                     2.,
          (Complex(0, -0.5) * sqrt(1 - pow(cth1, 2)) *
           sqrt(L(pow(mD, 2), pow(m1, 2), s23))) /
            sqrt(pow(mD, 2)),
          (Complex(0, -0.5) * cth1 * sqrt(L(pow(mD, 2), pow(m1, 2), s23))) /
            sqrt(pow(mD, 2));
        pmom[2] << (-((pow(m1, 2) - pow(mD, 2) - s23) *
                      (pow(m2, 2) - pow(m3, 2) + s23)) +
                    (cth1 * cth2 +
                     sqrt(1 - pow(cth1, 2)) * sqrt(1 - pow(cth2, 2))) *
                      sqrt(L(pow(mD, 2), pow(m1, 2), s23)) *
                      sqrt(L(s23, pow(m2, 2), pow(m3, 2)))) /
                     (4. * sqrt(pow(mD, 2)) * s23),
          (Complex(0, 0.25) *
           (sqrt(1 - pow(cth1, 2)) * (pow(m2, 2) - pow(m3, 2) + s23) *
              sqrt(L(pow(mD, 2), pow(m1, 2), s23)) +
            (pow(cth1, 2) * sqrt(1 - pow(cth2, 2)) *
               (pow(m1, 2) - pow(mD, 2) + 2 * sqrt(pow(mD, 2)) * sqrt(s23) -
                s23) +
             sqrt(1 - pow(cth2, 2)) * (-pow(m1, 2) + pow(mD, 2) + s23) +
             cth1 * sqrt(1 - pow(cth1, 2)) * cth2 *
               (-pow(m1, 2) + pow(mD, 2) - 2 * sqrt(pow(mD, 2)) * sqrt(s23) +
                s23)) *
              sqrt(L(s23, pow(m2, 2), pow(m3, 2))))) /
            (sqrt(pow(mD, 2)) * s23),
          (Complex(0, 0.25) *
           (cth1 * (pow(m2, 2) - pow(m3, 2) + s23) *
              sqrt(L(pow(mD, 2), pow(m1, 2), s23)) +
            (2 * cth2 * sqrt(pow(mD, 2)) * sqrt(s23) +
             pow(cth1, 2) * cth2 *
               (-pow(m1, 2) + pow(mD, 2) - 2 * sqrt(pow(mD, 2)) * sqrt(s23) +
                s23) +
             cth1 * sqrt(1 - pow(cth1, 2)) * sqrt(1 - pow(cth2, 2)) *
               (-pow(m1, 2) + pow(mD, 2) - 2 * sqrt(pow(mD, 2)) * sqrt(s23) +
                s23)) *
              sqrt(L(s23, pow(m2, 2), pow(m3, 2))))) /
            (sqrt(pow(mD, 2)) * s23);
        pmom[3] << (-((pow(m2, 2) - pow(m3, 2) - s23) *
                      (-pow(m1, 2) + pow(mD, 2) + s23)) -
                    (cth1 * cth2 +
                     sqrt(1 - pow(cth1, 2)) * sqrt(1 - pow(cth2, 2))) *
                      sqrt(L(pow(mD, 2), pow(m1, 2), s23)) *
                      sqrt(L(s23, pow(m2, 2), pow(m3, 2)))) /
        ...
        res << pmom[0].transpose() * pmom[0], pmom[0].transpose() * pmom[1],
          pmom[0].transpose() * pmom[2], pmom[0].transpose() * pmom[3],
          pmom[1].transpose() * pmom[0], pmom[1].transpose() * pmom[1],
          pmom[1].transpose() * pmom[2], pmom[1].transpose() * pmom[3],
          pmom[2].transpose() * pmom[0], pmom[2].transpose() * pmom[1],
          pmom[2].transpose() * pmom[2], pmom[2].transpose() * pmom[3],
          pmom[3].transpose() * pmom[0], pmom[3].transpose() * pmom[1],
          pmom[3].transpose() * pmom[2], pmom[3].transpose() * pmom[3];
    }
    if ((inp == 1) && (outp == 2)) {
        double mD = mm(0).real();
        double m1 = mm(1).real();
        ...
        pmom[0] << mD, 0., 0.;
        pmom[1] << (pow(m1, 2) - pow(m2, 2) + pow(mD, 2)) /
                     (2. * sqrt(pow(mD, 2))),
          0,
          -0.5 * (sqrt(L(pow(mD, 2), pow(m1, 2), pow(m2, 2)))) /
            sqrt(pow(mD, 2)) * 1i;
        pmom[2] << (pow(m1, 2) - pow(m2, 2) + pow(mD, 2)) /
                     (2. * sqrt(pow(mD, 2))),
          0,
          0.5 * (sqrt(L(pow(mD, 2), pow(m1, 2), pow(m2, 2)))) /
            sqrt(pow(mD, 2)) * 1i;
        ...
        res << pmom[0].transpose() * pmom[0], pmom[0].transpose() * pmom[1],
          pmom[0].transpose() * pmom[2], pmom[1].transpose() * pmom[0],
          pmom[1].transpose() * pmom[1], pmom[1].transpose() * pmom[2],
          pmom[2].transpose() * pmom[0], pmom[2].transpose() * pmom[1],
          pmom[2].transpose() * pmom[2];
    }
    if ((inp == 2) && (outp == 2)) {
        ...
        pmom[0] << ((1 + pow(m1, 2.) / ss - pow(m2, 2.) / ss) * sqrt(ss)) / 2.,
          0, -sqrt(L(ss, pow(m1, 2.), pow(m2, 2.))) / (2. * sqrt(ss)) * 1i;
        pmom[1] << ((1 - pow(m1, 2.) / ss + pow(m2, 2.) / ss) * sqrt(ss)) / 2.,
          0, sqrt(L(ss, pow(m1, 2), pow(m2, 2.))) / (2. * sqrt(ss)) * 1i;
        pmom[2] << ((1 + pow(m3, 2) / ss - pow(m4, 2) / ss) * sqrt(ss)) / 2.,
          -0.5 *
            (sqrt(1 - pow(cth, 2)) * sqrt(L(ss, pow(m3, 2.), pow(m4, 2.)))) /
            sqrt(ss) * 1i,
          -0.5 * (cth * sqrt(L(ss, pow(m3, 2.), pow(m4, 2.)))) / sqrt(ss) * 1i;
        pmom[3] << ((1 - pow(m3, 2.) / ss + pow(m4, 2.) / ss) * sqrt(ss)) / 2.,
          0.5 *
            (sqrt(1 - pow(cth, 2.)) * sqrt(L(ss, pow(m3, 2.), pow(m4, 2.)))) /
            sqrt(ss) * 1i,
          0.5 * (cth * sqrt(L(ss, pow(m3, 2.), pow(m4, 2.)))) / sqrt(ss) * 1i;
        ...
        for (size_t qi = 0; qi != pthSM.size(); ++qi) {
            for (size_t qj = 0; qj != pthSM.size(); ++qj) {
                ...
                if (charge == 0) {
                    ...
                    decay +=
                      (pid < ((int)f_G.size())) ? Decay(pid, pp).real() : 0.;
                }
            ...
            for (int ii = 0; ii < 13; ii++) {
                for (int jj = 0; jj < 13; jj++) {
                    for (int kk = 0; kk < 13; kk++) {
                        ...
                        decay +=
                          f_G[pid](pp) * jacD * as[jj] * as[kk] * as1[ii];
                    }
                }
            }
        }
...
double gamma(int& pid, double const& T, param_t& data)
{
    ...
    if ((in == 2) && (out == 2)) {
        ...
        for (ii = 0; ii < 12; ii++) {
            for (jj = 0; jj < 13; jj++) {
                s = pow(mi - mj, 2.) +
                    2. * mi * mj * (2. + xki[ii]); // xs1[ii](1.- xs1[ii]);
                ...
                jac = gg * pow(ms, 4.) * 2. * mi * mj * aki[ii] *
                      as[jj]; //*(1./(pow(xs1[ii] - 1.,2.)));
                amp = 0.;
                /*
                 for(int kk=0; kk!=f_G.size();++kk){
                 if(f_G[kk].name == pname){
                ...
                result += exp(xki[ii]) * Tsc / (256. * pow(M_PI, 5.)) * pin *
                          pout * amp.real() * 1. / sqrt(s) * jac *
                          boost::math::cyl_bessel_k(1, sqrt(s) / Tsc);
            }
        }
    }
    ...
    if (in == 1) {
        ...
        result    = gg * mD * mD / (2. * pow(M_PI, 2.)) * T *
                 boost::math::cyl_bessel_k(1, abs(mD) / T) * decay.real();
    }
    // cout << "\n  Res = " << result << endl;
    return result;
}
        ...
        for (size_t jj = 0; jj != pXSM.size(); ++jj) {
            ...
            for (size_t kk = 0; kk != pthSM.size(); ++kk) {
                for (size_t ll = 0; ll != pthSM.size(); ++ll) {
                    ...
                    // cout << f_G[pid].name << ">> : " << Prname(pX,2,2) <<
                    // endl;
...
void functh(param_t& pp, double hh, double zl, Eigen::VectorXcd& yn)
{
    ...
    for (size_t ii = 0; ii != pXSM.size(); ++ii) {
        for (size_t jj = 0; jj != pXSM.size(); ++jj) {
            ...
            for (auto& pid : pXX) {
                pr = (pid < ((int)f_G.size()))
                       ? gamma(pid, T, pp) / (s * Hb * xx)
                       : 0.;
                Xsec += (pr.real() > 0.) ? pr : 0.;
                // cout << endl << "Pr name1 = "<< f_G[pid].name << ": " << pr
                // << endl;
            }
            ...
            tempf(ii) +=
              -log(10) * xx *
              ((yth(ii) * yth(jj) / (yeq(ii) * yeq(jj)) - 1.) * Gam2(ii, jj));
        }
    }
    ...
    for (size_t ii = 0; ii != pXSM.size(); ++ii) {
        for (size_t kk = 0; kk != pXSM.size(); ++kk) {
            for (size_t jj = 0; jj != pXSM.size(); ++jj) {
                ...
                tempfp(ii, kk) += -log(10) * xx *
                                  ((del_ki * yth(jj) / (yeq(ii) * yeq(jj)) +
                                    del_kj * yth(ii) / (yeq(ii) * yeq(jj))) *
                                   Gam2(ii, jj));
            }
        }
    }
            ...
            if (ii <= jj) {
                ...
                for (auto& pid : pXX) {
                    pr = (pid < ((int)f_G.size()))
                           ...
                           : 0.;
                    Xsec += (pr.real() > 0.) ? pr : 0.;
                    // cout << endl << "Pr name1 = "<< f_G[pid].name << ": " <<
                    // pr << endl;
                }
                ...
                tempf(ii) +=
                  -log(10) * xx *
                  ((yth(ii) / yeq(ii) - yth(jj) / yeq(jj)) * Gam2(ii, jj));
            }
        }
    }
    ...
    for (size_t ii = 0; ii != pXSM.size(); ++ii) {
        for (size_t kk = 0; kk != pXSM.size(); ++kk) {
            for (size_t jj = 0; jj != pXSM.size(); ++jj) {
                ...
                tempfp(ii, kk) +=
                  -log(10) * xx *
                  ((del_ki / yeq(ii) - del_kj / yeq(jj)) * Gam2(ii, jj));
            }
        }
    }
    ...
    for (size_t ii = 0; ii != pXSM.size(); ++ii) {
        ...
        for (auto& pid : pXX) {
            pr = (pid < ((int)f_G.size())) ? gamma(pid, T, pp) / (s * Hb * xx)
                                           : 0.;
            Xsec += (pr.real() > 0.) ? pr : 0.;
            // cout << endl << "Pr name1 = "<< f_G[pid].name << ": " << pr <<
            // endl;
    ...
    if (nn > pXSM.size()) {
        ...
        for (size_t ii = 0; ii != pXSM.size(); ++ii) {
            ...
            cout << " fB.first : " << fB.first << " fAB.first : " << fAB.first
                 << endl;
            cout << " fB.second : " << fB.second
                 << " fAB.second : " << fAB.second << endl;
            cout << "XsecB : " << XsecB;
            ...
            tempf(nn - 1) +=
              log(10) * xx *
              ((yth(ii) / yeq(ii) - 1.) * CPOdd -
               yth(nn - 1) / yeq(nn - 1) *
                 (CPEven.first * yth(ii) / yeq(ii) + CPEven.second));
            tempfp(nn - 1, nn - 1) +=
              -log(10) * xx / yeq(nn - 1) *
              ((CPEven.first * yth(ii) / yeq(ii) + CPEven.second));
            tempfp(nn - 1, ii) +=
              log(10) * xx *
              (CPOdd / yeq(ii) -
               yth(nn - 1) / yeq(nn - 1) * (CPEven.first / yeq(ii)));
        }
        ...
        for (size_t ii = 0; ii != pXSM.size(); ++ii) {
            for (size_t jj = 0; jj != pXSM.size(); ++jj) {
                if (ii <= jj) {
                    ...
                    GCPEven1b(ii, jj) +=
                      fB.second * XsecB + fAB.second * XsecAB;

                    if (ii < jj) {
                        GCPOdd1(jj, ii) += XsecB - XsecAB;
                        GCPEven1a(jj, ii) +=
                          fB.first * XsecB + fAB.first * XsecAB;
                        GCPEven1b(jj, ii) +=
                          fB.second * XsecB + fAB.second * XsecAB;
                    }
                ...
                tempf(nn - 1) +=
                  log(10) * xx *
                  ((yth(ii) * yth(jj) / (yeq(ii) * yeq(jj)) - 1.) *
                     GCPOdd1(ii, jj) -
                   yth(nn - 1) / yeq(nn - 1) *
                     (GCPEven1a(ii, jj) * yth(ii) * yth(jj) /
                        (yeq(ii) * yeq(jj)) +
                      GCPEven1b(ii, jj)));

                tempfp(nn - 1, nn - 1) += -log(10) * xx / yeq(nn - 1) *
                                          ((GCPEven1a(ii, jj) * yth(ii) *
                                              yth(jj) / (yeq(ii) * yeq(jj)) +
                                            GCPEven1b(ii, jj)));

                tempf(nn - 1) +=
                  log(10) * xx *
                  ((yth(ii) / yeq(ii) - yth(jj) / yeq(jj)) * GCPOdd2(ii, jj) -
                   yth(nn - 1) / yeq(nn - 1) *
                     (GCPEven2a(ii, jj) * yth(ii) / yeq(ii) +
                      GCPEven2b(ii, jj) * yth(jj) / yeq(jj)));
            ...
            for (size_t ii = 0; ii != pXSM.size(); ++ii) {
                for (size_t jj = 0; jj != pXSM.size(); ++jj) {
                    ...
                    tempfp(nn - 1, kk) +=
                      log(10) * xx *
                      ((del_ki * yth(jj) / (yeq(ii) * yeq(jj)) +
                        yth(ii) * del_kj / (yeq(ii) * yeq(jj))) *
                         GCPOdd1(ii, jj) -
                       yth(nn - 1) / yeq(nn - 1) *
                         (GCPEven1a(ii, jj) * del_ki * yth(jj) /
                            (yeq(ii) * yeq(jj)) +
                          GCPEven1a(ii, jj) * yth(ii) * del_kj /
                            (yeq(ii) * yeq(jj))));

                    tempfp(nn - 1, kk) +=
                      log(10) * xx *
                      ((del_ki / yeq(ii) - del_kj / yeq(jj)) * GCPOdd2(ii, jj) -
                       yth(nn - 1) / yeq(nn - 1) *
                         (GCPEven2a(ii, jj) * del_ki / yeq(ii) +
                          GCPEven2b(ii, jj) * del_kj / yeq(jj)));
                }
            }
        }
    }
    ...
    if ((ath.determinant() != 0) && (!isnan(ath.determinant())) &&
        (!isinf(ath.determinant()))) {
        ypth = expM(ath * hh, dim) * yth +
               tempfp.inverse() *
                 (expM(ath * hh, dim) - MatrixXd::Identity(dim, dim)) * bth;
    } else {
        ypth = yth;
    }
          ...
          param_t&          p)
{
    ...
    for (i = 0; i < iter; i++) {
        ...
        cout << endl;
        /*
         cout << pow(10., z[i]) << "\t" << m_XX(p)/pow(10., z[i])
         <<"\t"<<sqrt(p.muS2.real())*sbyrho*Yeq(1., sqrt(p.muS2.real()),
         T)<<"\t"<<sqrt(p.muS2.real())*sbyrho*(yn[0][i]); cout
         <<"\t"<<p.m_X.real()*sbyrho*Yeq(2., p.m_X.real(), T) << "\t" <<
         (p.m_X.real())*sbyrho*(yn[1][i]) << " "
         << 2./3.*sbyrho*yn[2][i] << endl; cout << "Ωh^2 =  " <<
         p.m_X.real()*sbyrho*(yn[1][i]) +
         sqrt(p.muS2.real())*sbyrho*(yn[0][i])<< endl;

         cout << pow(10., z[i]) << "\t" << p.m_X.real()/pow(10., z[i])
         <<"\t"<<sqrt(p.muS2.real())*sbyrho*Yeq(1., sqrt(p.muS2.real()),
         T)<<"\t"<<sqrt(p.muS2.real())*sbyrho*(yn[0][i])*Yeq(1.,
         sqrt(p.muS2.real()),T); cout
         <<"\t"<<p.m_X.real()*sbyrho*Yeq(2., p.m_X.real(), T) << "\t" <<
         (p.m_X.real())*sbyrho*(yn[1][i])*Yeq(2., p.m_X.real(), T) << " "
         << 2./3.*sbyrho*yn[2][i]
         << endl; cout << "Ωh^2 =  " << p.m_X.real()*sbyrho*(yn[1][i])*Yeq(2.,
         p.m_X.real(), T) + sqrt(p.muS2.real())*sbyrho*(yn[0][i])*Yeq(1.,
         sqrt(p.muS2.real()),T) << endl;*/
    }
}
